...
export const AdvancedEditor: React.FC<AdvancedEditorProps> = ({
  ...
  const changeEditorBg = (color: string) => {
    ...
    try {
      ...
      try {
        localStorage.setItem('editorBg', color)
      } catch {}
    } catch (err) {
  ...
  const acceptSuggestionAtCursor = useCallback(() => {
    ...
    console.debug(
      '[AdvancedEditor] acceptSuggestionAtCursor called, text length=',
      text.length
    )
  ...
  useEffect(() => {
    if (suggestionPreview) {
      console.debug(
        '[AdvancedEditor] suggestionPreview set (len):',
        suggestionPreview.length
      )
    } else {
      ...
      if (e.altKey && e.key === 'Enter') {
        ...
        try {
          ...
          if (editorNode && editorNode.contains(active)) {
            e.preventDefault()
            console.debug(
              '[AdvancedEditor] doc-level Alt+Enter detected, accepting suggestion'
            )
      ...
      if (response.data.length > 0 && !selectedFile) {
        ...
        setCurrentLanguage(
          response.data[0].language ||
            getLanguageFromFileName(response.data[0].name)
        )
      }
    } catch (error) {
  ...
  const createFile = async () => {
    ...
    try {
      ...
      const filePath = createInPath
        ? `${createInPath.replace(/\/$/, '')}/${newFileName}`
        : newFileName
      ...
      setCode('')
      setCurrentLanguage(
        newFile.language || getLanguageFromFileName(newFile.name)
      )
      setNewFileName('')
  ...
  const createFolder = async () => {
    ...
    try {
      ...
      const existing = files.find(
        f => (f.path || f.name) === `${folderPath}/.gitkeep`
      )
    ...
    try {
      ...
      if (selectedFile?.id === fileId) {
        if (updatedFiles.length > 0) {
          ...
          setCurrentLanguage(
            updatedFiles[0].language ||
              getLanguageFromFileName(updatedFiles[0].name)
          )
        } else {
      ...
      .forEach(folderName => {
        ...
        elements.push(
          <div key={folderPath} className="pl-0">
            <div className="flex items-center justify-between p-1 rounded hover:bg-zinc-800">
              <div className="flex items-center space-x-2 flex-1">
                <button
                  ...
                  className={`w-4 h-4 flex items-center justify-center rounded transition-transform ${isOpen ? 'rotate-90' : ''} text-zinc-300 hover:text-white`}
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="p-1 text-xs rounded bg-zinc-800/50 hover:bg-zinc-700 text-zinc-200"
            ...
            >
              ...
              <div className="flex items-center space-x-2">
                <select
                  ...
                  className="bg-transparent text-xs border border-zinc-700/20 rounded p-1 text-zinc-300"
                  title="Move file to folder"
                >
                  <option value="">(root)</option>
                  {getAllFolderPaths().map(fp => (
                    <option key={fp} value={`${fp}/${file.name}`}>
                      {fp}
                    </option>
                  ))}
                </select>
                <button
                  ...
                  className="p-1 hover:bg-red-500/20 rounded text-zinc-500 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
                >
  ...
  const extractCodeFromSuggestion = (suggestion: string | undefined | null) => {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (
        /[;{}=<>\(\)\[\]"'`]|\bconsole\.|\breturn\b|\bfunction\b|\bclass\b/.test(
          ln
        )
      ) {
  ...
  const detectLanguageFromCode = (snippet: string): string | null => {
    ...
    if (
      ...
      /\bfunction\b/.test(s) ||
      /=>/.test(s) ||
      /\bconst\b|\blet\b|\bvar\b/.test(s) ||
      /module\.exports|export\s+default/.test(s)
    )
      return 'javascript'
    if (/interface\s+\w+/.test(s) || (/:\s*\w+/m.test(s) && /export/.test(s)))
      return 'typescript'
    ...
    // C/C++/Java hints
    if (/#include\s+<|#include\s+"/.test(s) || /printf\(|std::/.test(s))
      return 'c'
    if (/public\s+class\s+\w+/.test(s) || /System\.out\.println\(/.test(s))
      return 'java'
  ...
  const handleEditorDidMount = useCallback(
    (editor: any, monaco: any) => {
      ...
      const createWidgetDom = (text: string, langBadge?: string) => {
        ...
        el.className =
          'inline-suggestion text-zinc-400 text-sm select-none flex items-center space-x-2'
      ...
      try {
        ...
        if (
          monaco &&
          typeof monaco.KeyMod !== 'undefined' &&
          typeof monaco.KeyCode !== 'undefined'
        ) {
  ...
  const handleCodeChange = useCallback(
    (value: string | undefined) => {
      ...
      autosaveTimerRef.current = setTimeout(() => {
        ...
        if (selectedFile) {
          console.debug(
            '[AdvancedEditor] Autosave triggered for file:',
            selectedFile.id
          )
          try {
            saveFile(true)
            ...
            console.error(
              '[AdvancedEditor] Autosave saveFile threw error:',
              err
            )
          }
        } else {
      ...
      if (response.ok) {
        ...
        try {
          ...
          if (
            ...
            setSuggestions(
              analysisResult.suggestions.map((suggestion: any) => ({
                code: extractCodeFromSuggestion(
                  suggestion.code || suggestion.text || ''
                ),
                explanation:
          ...
          if (analysisResult.bugs && Array.isArray(analysisResult.bugs)) {
            setBugs(
              analysisResult.bugs.map((bug: any) => ({
                line: bug.line || 1,
                type: bug.type || 'logic',
                severity: bug.severity || 'medium',
                description:
                  bug.description || bug.message || 'Potential issue detected',
                suggestion:
                  bug.suggestion || bug.fix || 'Consider reviewing this code',
              }))
            )
          }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (detected && detected !== currentLanguage) {
          console.debug(
            '[AdvancedEditor] Detected language mismatch:',
            detected,
            'expected:',
            currentLanguage
          )
          ...
          try {
            ...
            if (retryResp.ok) {
              ...
              try {
                const parsed = JSON.parse(retryData.suggestion)
                retryCode =
                  parsed.code || parsed.completion || retryData.suggestion
              } catch {}
        ...
        const langForBadge =
          detectLanguageFromCode(completionCode) || currentLanguage
    ...
    try {
      ...
      if (response.ok) {
        ...
        const explanationText =
          extractCodeFromSuggestion(explanation) || explanation
  ...
  const saveFile = async (quiet: boolean = false) => {
    ...
    console.debug('[AdvancedEditor] saveFile called', {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#09090b] text-zinc-100">
      {/* Toolbar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 py-2.5 border-b border-zinc-800 bg-zinc-900 gap-3 sm:gap-0">
        <div className="flex items-center space-x-4">
          {/* Back to Dashboard button */}
          <button
            title="Back to Dashboard"
            onClick={() => {
              if (typeof (onBack as any) === 'function') {
                ;(onBack as any)()
              } else {
                window.history.back()
              }
            }}
            className="mr-2 p-1 rounded bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-zinc-300"
          >
            ←
          </button>
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-1 bg-zinc-800 border border-zinc-700 text-xs rounded-lg text-zinc-300">
              {currentLanguage}
            </span>
        ...
        <div className="flex items-center space-x-2 flex-wrap gap-2">
          <button
            className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 border border-zinc-700 text-sm rounded-lg flex items-center space-x-2 text-zinc-200 hover:text-white transition-colors hover:bg-zinc-600 disabled:opacity-50 disabled:cursor-not-allowed"
            onClick={analyzeCode}
          ...
          <button
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-sm rounded-lg flex items-center space-x-2 text-white transition-colors"
            onClick={() => setShowCommandPalette(true)}
          >
            <Sparkles className="w-4 h-4" />
            <span>AI</span>
            <kbd className="text-[10px] bg-indigo-700 px-1 py-0.5 rounded ml-1">⌘K</kbd>
          </button>

          <button
            className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 border border-zinc-700 text-sm rounded-lg flex items-center space-x-2 text-zinc-200 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            onClick={() => saveFile()}
          ...
          <div className="flex items-center space-x-2">
            <label className="text-xs text-zinc-300">Editor BG</label>
            <select
              value={editorBg}
              onChange={e => changeEditorBg(e.target.value)}
              className="bg-zinc-800 border border-zinc-700 text-sm rounded p-1 text-white"
            >
              {colorOptions.map(c => (
      ...
      <div className="flex-1 flex">
        {/* File Explorer */}
        <div className="w-64 border-r border-zinc-800 bg-zinc-900 relative z-10">
                ...
                {showNewFolderInput ? (
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="p-1 pl-2 pr-2 bg-zinc-800 border border-zinc-700 rounded text-white placeholder-zinc-600 text-sm"
                      onKeyDown={e => {
                        if (e.key === 'Enter') {
                          createFolder()
                    ...
                    <button
                      ...
                      className="px-2 py-1 bg-zinc-700 hover:bg-zinc-600 rounded border border-zinc-700 text-sm"
                    >
                      Create
                    </button>
                    <button
          ...
          <div className="overflow-y-auto h-full">
            {loading ? (
              ...
              <div className="p-4 text-center text-zinc-400">
                ...
                <div className="mt-4">
                  <h4 className="text-sm text-zinc-200 mb-2">Create folder</h4>
                  <div className="flex space-x-2">
                    <input
                      ...
                      onChange={e => setNewFolderName(e.target.value)}
                      placeholder="folder/name"
                      className="flex-1 p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white placeholder-zinc-600"
                    />
                    <button
                      onClick={createFolder}
                      className="p-2 bg-zinc-700 hover:bg-zinc-600 rounded border border-zinc-700"
                    >
                      Create
                    </button>
                  </div>
              ...
              <div className="p-2">{renderTree(buildFileTree(files))}</div>
            )}
          </div>
        </div>
        ...
        {/* AI Panel */}
        <div className="w-96 border-l border-zinc-800 bg-zinc-900 relative z-10 hidden lg:block">
          {/* Tab Navigation */}
          <div className="flex border-b border-zinc-800">
            <button
              className={`flex-1 px-4 py-3 text-sm flex items-center justify-center space-x-2 transition-colors ${
                activeTab === 'suggestions'
                  ? 'bg-zinc-700 text-white border-b-2 border-indigo-500'
                  : 'text-zinc-400 hover:text-white hover:bg-zinc-800'
              }`}
          ...
          <div className="h-full overflow-y-auto p-4 space-y-4">
            {isAnalyzing && (
              <div className="bg-zinc-800 border border-zinc-800 rounded-lg p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-400">
                      AI is processing your code for suggestions and issues
                    </p>
                  </div>
                </div>
              </div>
                ...
                {suggestions.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {suggestions.map((suggestion, index) => (
                      <div
                        key={index}
                        className="bg-zinc-800 border border-zinc-800 rounded-lg p-4 hover:bg-zinc-700 transition-colors hover:bg-zinc-600"
                      >
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-xs bg-zinc-700 border border-zinc-700 px-3 py-1 rounded-lg text-zinc-200">
                            {Math.round(suggestion.confidence * 100)}%
                            confidence
                          </span>
                          <button
                            className="text-zinc-400 hover:text-white transition-colors p-1 hover:bg-zinc-700 rounded"
                        ...
                        <button
                          className="w-full bg-zinc-700 hover:bg-zinc-600 border border-zinc-700 text-zinc-200 hover:text-white text-sm py-2 rounded-lg transition-colors hover:bg-zinc-600"
                ...
                {bugs.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {bugs.map((bug, index) => (
                      <div
                        key={index}
                        className="bg-zinc-800 border border-zinc-800 rounded-lg p-4 hover:bg-zinc-700 transition-colors"
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-2">
                            <div
                              className={`p-1 rounded ${getSeverityColor(bug.severity).replace('bg-', 'bg-').replace('-500', '-500/20')} border border-zinc-700`}
                            >
                              {getBugIcon(bug.type)}
                            </div>
                            <span
                              className={`text-xs px-3 py-1 rounded-lg text-white border ${getSeverityColor(bug.severity).replace('bg-', 'border-').replace('-500', '-400/50')} ${getSeverityColor(bug.severity).replace('bg-', 'text-').replace('-500', '-100')}`}
                            >
                              {bug.severity}
                            </span>
                          </div>
            ...
            {activeTab === 'chat' && (
              <div className="animate-in slide-in-from-right-5 duration-300">
                <div className="flex flex-col h-96">
                  <div className="flex-1 overflow-y-auto p-2 space-y-2 bg-zinc-950 rounded">
                    {chatMessages.length === 0 ? (
                      ...
                      chatMessages.map((m, i) => (
                        <div
                          key={i}
                          className={`p-2 rounded ${m.role === 'assistant' ? 'bg-zinc-800' : 'bg-zinc-800/50'}`}
                        >
                          ...
                          {m.role === 'assistant' && (
                            <div className="mt-2 flex space-x-2">
                              <button
                                className="text-sm px-2 py-1 bg-zinc-700 hover:bg-zinc-600 rounded"
                                onClick={() => {
                                  const code = extractCodeFromSuggestion(m.text)
                                  if (!code) {
                                    addToast(
                                      'No code block found in assistant message',
                                      'error'
                                    )
                                    return
                                  }
                              ...
                              <button
                                className="text-sm px-2 py-1 bg-zinc-800/50 hover:bg-zinc-700 rounded"
                                onClick={() => {
                  ...
                  <div className="mt-2">
                    <textarea
                      ...
                      rows={3}
                      className="w-full p-2 bg-zinc-800 border border-zinc-800 rounded text-white placeholder-zinc-600"
              ...
              <div className="animate-in slide-in-from-right-5 duration-300">
                {complexity ? (
                  <div className="space-y-6">
                    <div className="bg-zinc-800 border border-zinc-800 rounded-lg p-4">
                      <h3 className="text-sm font-semibold mb-4 text-zinc-200 flex items-center space-x-2">
                        <Zap className="w-4 h-4" />
                        <span>Complexity Analysis</span>
                      </h3>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-zinc-700 border border-zinc-700 rounded-lg p-3">
                        ...
                        <div className="bg-zinc-700 border border-zinc-700 rounded-lg p-3">
                          ...
                          <span
                            className={`text-xs px-3 py-1 rounded-lg text-white border ${
                    ...
                    <div className="bg-zinc-800 border border-zinc-800 rounded-lg p-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center bg-zinc-700 border border-zinc-700 rounded-lg p-3">
                        ...
                        <div className="flex justify-between items-center bg-zinc-700 border border-zinc-700 rounded-lg p-3">
                          ...
                          <span className="text-xs bg-zinc-700 border border-zinc-600 px-3 py-1 rounded-lg text-zinc-200">
                            {suggestions.length}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="text-center text-zinc-400 py-12">
                    ...
                    <p className="text-xs mt-1">
                      Run code analysis to see metrics
                    </p>
                  </div>
                )}
              </div>
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {toasts.map(toast => (
          <div
            key={toast.id}
            className={`max-w-sm p-4 rounded-lg shadow-lg border transition-colors ${
              toast.type === 'success'
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 w-96">
            ...
            <input
              ...
              className="w-full p-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white placeholder-zinc-500 focus:border-zinc-700 focus:outline-none"
              onKeyDown={e => {
                if (e.key === 'Enter') {
                  createFile()
                } else if (e.key === 'Escape') {
            ...
            <div className="flex space-x-3 mt-4">
              <button
                ...
                className="flex-1 bg-zinc-700 hover:bg-zinc-600 disabled:opacity-50 disabled:cursor-not-allowed border border-zinc-700 text-white py-2 px-4 rounded-lg transition-colors"
              >
                Create
              </button>
              <button
                ...
                className="flex-1 border border-zinc-700 text-zinc-300 hover:text-white hover:bg-zinc-800 py-2 px-4 rounded-lg transition-colors"
              >
                Cancel
              </button>
            </div>
      ...
      <CommandPalette
        ...
        onApplyCode={(newCode) => {
          ...
          addToast('Code applied from command palette', 'success')
        }}
      />
    </div>
  )
